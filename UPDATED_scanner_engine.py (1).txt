...
def calculate_liquidity_based_levels(df, direction='long', is_large_cap=True):
    ...
    if direction == 'long':
        ...
        if len(all_bsl) >= 2:
            tp1 = all_bsl[0]
            tp2 = all_bsl[1]
            # FIX: Ensure TP2 is meaningfully different from TP1 (at least 2% apart)
            if abs(tp2 - tp1) / tp1 < 0.02:
                # TP2 too close to TP1 — extend with next available or 1.5x extension
                if len(all_bsl) >= 3 and (all_bsl[2] - tp1) / tp1 >= 0.02:
                    tp2 = all_bsl[2]
                else:
                    # Extend TP2 by projecting the range
                    extension = (tp1 - current_price) * 1.5
                    tp2 = tp1 + extension
            tp_reason = f"TP1={tp1:.2f} (nearest BSL), TP2={tp2:.2f} (next liquidity)"
        elif len(all_bsl) == 1:
            tp1 = all_bsl[0]
            # FIX: TP2 = either 52W high (if higher & different) OR TP1 + additional projection
            if wk52_high > tp1 * 1.02:
                tp2 = wk52_high
                tp_reason = f"TP1={tp1:.2f} (BSL), TP2={tp2:.2f} (52W High)"
            else:
                # Project TP2 = TP1 + (TP1 - Entry) — same magnitude extension
                extension = (tp1 - current_price)
                tp2 = tp1 + extension
                tp_reason = f"TP1={tp1:.2f} (BSL), TP2={tp2:.2f} (projected extension)"
        else:
            # ⚠️ No BSL above — stock likely at ATH / no room
            # This is risky — use conservative fixed R:R
            tp1 = current_price + (risk * 2)
            tp2 = current_price + (risk * 3.5)
            tp_reason = "⚠️ No BSL above (at ATH) — projected 1:2 & 1:3.5 R:R"
        ...
        if len(all_ssl) >= 2:
            tp1 = all_ssl[0]
            tp2 = all_ssl[1]
            # FIX: Ensure TP2 differ from TP1
            if abs(tp1 - tp2) / tp1 < 0.02:
                if len(all_ssl) >= 3 and (tp1 - all_ssl[2]) / tp1 >= 0.02:
                    tp2 = all_ssl[2]
                else:
                    extension = (current_price - tp1) * 1.5
                    tp2 = tp1 - extension
            tp_reason = f"TP1={tp1:.2f} (nearest SSL), TP2={tp2:.2f} (next liquidity)"
        elif len(all_ssl) == 1:
            tp1 = all_ssl[0]
            if wk52_low < tp1 * 0.98:
                tp2 = wk52_low
                tp_reason = f"TP1={tp1:.2f} (SSL), TP2={tp2:.2f} (52W Low)"
            else:
                extension = (current_price - tp1)
                tp2 = tp1 - extension
                tp_reason = f"TP1={tp1:.2f} (SSL), TP2={tp2:.2f} (projected extension)"
        else:
            tp1 = current_price - (risk * 2)
            tp2 = current_price - (risk * 3.5)
            tp_reason = "⚠️ No SSL below (at ATL) — projected 1:2 & 1:3.5 R:R"
...
def scan_stock(symbol, info, include_fundamental=True):
    ...
    try:
        ...
        # === SMART DIRECTION CORRECTION ===
        # If LONG selected but NO BSL exists (stock at ATH) → this is risky
        # Check if flipping direction gives better R:R
        if direction == 'long' and not levels_data.get("all_bsl_levels"):
            # Try SHORT direction — see if it gives better setup
            short_levels = calculate_liquidity_based_levels(df, 'short', is_large_cap)
            if short_levels.get("all_ssl_levels") and short_levels["rr1"] >= 1.5:
                # SHORT has better liquidity map — flip direction
                direction = 'short'
                levels_data = short_levels
                tech_signals.append(("Direction Flip", "⚠️ No BSL above — SHORT setup better", "warn"))
        elif direction == 'short' and not levels_data.get("all_ssl_levels"):
            long_levels = calculate_liquidity_based_levels(df, 'long', is_large_cap)
            if long_levels.get("all_bsl_levels") and long_levels["rr1"] >= 1.5:
                direction = 'long'
                levels_data = long_levels
                tech_signals.append(("Direction Flip", "⚠️ No SSL below — LONG setup better", "warn"))
