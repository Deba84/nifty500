...
def add_fundamental_safe(item: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    try:
        ...
        item["final_score"] = item["setup_score"]
        item["base_status"] = status_from_score(item["setup_score"])
        item["final_status"] = item["base_status"]
        item["priority"] = item["setup_score"]
        if score == 2:
...
def candidate_sort_key(item: Dict[str, Any], use_final: bool = False) -> Tuple[int, float, float, float]:
    score_key = "final_score" if use_final else "technical_score"
...
def _send_results(
    ...
    if not visible:
        send_telegram(
            f"⚠️ <b>NO VALID v6.2 SETUPS TODAY</b>\n\n"
    ...
    breadth_text = (
        ...
        "⚠️ <i>এটি scanner breadth; পুরো Nifty market direction নয়।</i>"
    )
    if ai_response.get("breadth_commentary_bn"):
        breadth_text += f"\n🤖 {_esc(ai_response['breadth_commentary_bn'])}"
    send_telegram(breadth_text)

    categories = [
        ("✅ <b>TRIGGER READY — CHART VERIFY</b>", trigger),
        ("🔁 <b>POST-SWEEP — CONFIRMATION PENDING</b>", confirming),
        ("🎯 <b>PRE-SWEEP: AT LEVEL</b>", at_level),
        ("🔥 <b>PRE-SWEEP: VERY CLOSE</b>", very_close),
        ("⚡ <b>PRE-SWEEP: CLOSE WATCHLIST</b>", close),
    ]
    for title, items in categories:
        ...
        send_telegram(f"\n{title} ({len(items)})")
        for rank, item in enumerate(ordered[:MAX_ALERTS_PER_CATEGORY], 1):
