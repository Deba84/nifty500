"""Scanner engine with Technical + Fundamental analysis (v2)."""
...
def calculate_technical_score(df, info):
    """Technical scoring (max 10 points)."""
    score = 0
    signals = []
    reasons = []
...
def scan_stock(symbol, info, include_fundamental=True):
    """Main scan combining Technical + Fundamental."""
    try:
        ...
        # === TOTAL SCORE ===
        total_score = tech_score + fund_score  # Max: 11 tech + 4 fund = 15
        ...
        # A+ SUPER = Technical A+ AND Fundamental Strong (tech ≥ 8 AND fund ≥ 3)
        # A+ = Technical strong only (tech ≥ 8)
        # A = Good technical (tech ≥ 6)
        # WATCH = tech ≥ 4
        if tech_score >= 8 and fund_score >= 3:
