...
def scan_stock(symbol, info, include_fundamental=True, min_fund_score=3):
    ...
    1. Are approaching a major untapped liquidity level (FAST check first)
    2. Are fundamentally strong (SLOW check only if step 1 passes)
    """
    try:
        ...
        # ============ 1. FIND UNTAPPED LIQUIDITY (FAST) ============
        ...
        # ============ 4. FUNDAMENTAL CHECK (SLOW - only if above passed) ============
        # This is expensive (fetches Screener.in), so only for candidates
        fund_data = None
        fund_score = 0
        fund_reasons = []
