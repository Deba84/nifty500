...
def _count_near_test_episodes(df: pd.DataFrame, level: Dict[str, Any], end_idx: int) -> int:
    ...
    # Performance optimization: convert column to NumPy array once to eliminate
    # pandas Series overhead (`.iloc[i]`) inside the loop (~20x faster per call).
    ...
    for i in range(start, end + 1):
        if any(abs(i - t) <= 1 for t in touch_set):
            continue
        if level["side"] == "BSL":
            distance = max(0.0, (target - prices[i]) / target * 100)
        else:
            distance = max(0.0, (prices[i] - target) / target * 100)
