"""
Scanner v6 FINAL — 100% Batch Download, ZERO individual calls
- All 500 stocks in single batch (10 seconds)
- No fallback to individual calls (which caused timeout)
...
# ============ BATCH DOWNLOAD IN CHUNKS (KEY!) ============

def batch_download_all(symbols_list, period="1y", chunk_size=100):
    """
    Download in CHUNKS to avoid single-batch failures.
    100 stocks per chunk × 5 chunks = 500 stocks
    Each chunk: ~5-10 sec, NO individual fallback!
    """
    ...
    for i in range(0, len(symbols_list), chunk_size):
        ...
        print(f"  Chunk {chunk_num}/{total_chunks}: {len(chunk)} stocks...")
...
# NO OLD scan_stock() function — completely removed!
# This ensures daily_scan.py CANNOT accidentally use individual calls.
