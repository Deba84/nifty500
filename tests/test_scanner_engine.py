...
class LiquidityHistoryTests(unittest.TestCase):
    ...
    def test_fresh_standalone_52w_level_is_immature(self):
        level = {
            "primary_type": "52W_HIGH",
            "source_types": ["52W_HIGH"],
            "touch_count": 1,
            "confluence_count": 0,
            "formation_idx": 100,
            "age_bars": 0,
        }
        self.assertTrue(is_immature_singleton_level(level))
        level["age_bars"] = 3
        self.assertFalse(is_immature_singleton_level(level))
        level["age_bars"] = 0
        level["source_types"].append("SWING_HIGH")
        self.assertFalse(is_immature_singleton_level(level))
